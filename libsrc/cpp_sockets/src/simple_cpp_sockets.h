...
namespace simple_cpp_sockets {
	...
	typedef std::function<void(void*, size_t)> ServerRecieveCallbackFn;

	class UDPClient : public Socket
	{
	public:
		UDPClient(u_short port = 8000, const std::string& ip_address = "127.0.0.1");
		ssize_t send_message(const void* data, size_t size);
	};
	...
	public:
		...
		int socket_bind();
		void listen(size_t buffer_size, ServerRecieveCallbackFn callback);
	};
	...
	ssize_t UDPClient::send_message(const void* data, size_t size) {
		return sendto(m_socket, (const char*)data, size, 0, reinterpret_cast<sockaddr*>(&m_addr), sizeof(m_addr));
	};
	...
	void UDPServer::listen(size_t buffer_size, ServerRecieveCallbackFn callback) {
		sockaddr_in client;
		...
		while (true) {
			...
			if (recv_len == SOCKET_ERROR) {
				...
				callback(message_buffer, recv_len);
			}
		}
