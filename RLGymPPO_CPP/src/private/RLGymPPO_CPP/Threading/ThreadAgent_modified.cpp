...
void _RunFunc(ThreadAgent* ta) {
	...
	while (ta->shouldRun) {
		...
		try {
			// Check if we should use a policy from the pool for some players
			if (mgr->policyPool && mgr->policyPool->ShouldUsePooledPolicy()) {
				// Get a random policy from the pool
				DiscretePolicy* pooledPolicy = mgr->policyPool->GetRandomPolicy();
				
				// If we have a valid policy from the pool, use it for some players
				if (pooledPolicy) {
					// For now, we'll use a simple approach: use the current policy for all players
					// but for the second team (if it exists), use the pooled policy
					// This assumes a standard 1v1, 2v2, or 3v3 setup
					
					// First, determine how many players are in the first team
					int firstTeamSize = 0;
					for (auto game : games) {
						firstTeamSize = game->match->playerAmount / 2; // Assuming even teams
						break; // Just use the first game to determine team size
					}
					
					// Get actions for first team using current policy
					auto firstTeamObs = curObsTensorDevice.slice(0, 0, firstTeamSize);
					auto firstTeamActions = policy->GetAction(firstTeamObs, deterministic);
					
					// Get actions for second team using pooled policy
					auto secondTeamObs = curObsTensorDevice.slice(0, firstTeamSize);
					auto secondTeamActions = pooledPolicy->GetAction(secondTeamObs, deterministic);
					
					// Combine the actions
					actionResults.action = torch::cat({firstTeamActions.action, secondTeamActions.action});
					actionResults.logProb = torch::cat({firstTeamActions.logProb, secondTeamActions.logProb});
				} else {
					// If no pooled policy is available, fall back to the current policy
					actionResults = policy->GetAction(curObsTensorDevice, deterministic);
				}
			} else {
				// Use the current policy for all players
				actionResults = policy->GetAction(curObsTensorDevice, deterministic);
			}
		} catch (std::exception& e) {
